...
* **Live Dashboard:** Opt-in web UI streams live metrics (RPS, users, latency charts) over SSE while a test runs. Optional authenticated control lets you Start, Stop, and set target users from the browser—see [Live Dashboard](https://book.goose.rs/controller/dashboard.html) in the Goose Book.
...
* [**Live Dashboard**](https://book.goose.rs/controller/dashboard.html) Observe (and optionally control) a running load test from the browser (`--dashboard`, `--dashboard-control`).
