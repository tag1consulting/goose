...
async fn test_baseline_file_validation_success() {
    ...
    let temp_path = "/tmp/test_baseline_valid.json";
...
async fn test_baseline_file_validation_failure() {
    ...
    let temp_path = "/tmp/test_baseline_invalid.json";
...
async fn test_baseline_file_not_found() {
    ...
    let server = MockServer::start();
    let configuration = common::build_configuration(&server, vec!["--baseline-file", "nonexistent_baseline.json"]);
...
async fn test_baseline_empty_file() {
    // Create empty file
    let temp_path = "/tmp/test_baseline_empty.json";
...
async fn test_baseline_invalid_json() {
    ...
    let temp_path = "/tmp/test_baseline_malformed.json";
...
async fn test_baseline_missing_required_fields() {
    ...
    let temp_path = "/tmp/test_baseline_minimal.json";
...
async fn test_baseline_integration_with_load_test() {
    ...
    let temp_path = "/tmp/test_baseline_integration.json";
    create_test_baseline(Path::new(temp_path), true).expect("Failed to create test baseline");

    // Build configuration with baseline file
    let configuration = common::build_configuration(
        &server,
        vec!["--baseline-file", temp_path, "--report-file", "/tmp/goose-test-report.json"]
    );

    // Create scenario with proper transaction
    let scenario = scenario!("test_scenario")
        .register_transaction(transaction!(simple_transaction));

    // Build and run load test
    let goose_attack = common::build_load_test(configuration, vec![scenario], None, None);
    let _goose_metrics = common::run_load_test(goose_attack, None).await;

    // Clean up
    common::cleanup_files(vec!["/tmp/goose-test-report.json", temp_path]);
}

#[tokio::test]
async fn test_baseline_file_content_validation() {
    // Create a temporary valid baseline file
    let temp_path = "/tmp/test_baseline_content.json";
    create_test_baseline(Path::new(temp_path), true).expect("Failed to create test baseline");
    ...
    assert!(parsed.get("started").is_some(), "Baseline should contain started timestamp");
    assert!(parsed.get("elapsed").is_some(), "Baseline should contain elapsed time");
    assert!(parsed.get("users").is_some(), "Baseline should contain user count");
    assert!(parsed.get("requests").is_some(), "Baseline should contain request metrics");
    assert!(parsed.get("responses").is_some(), "Baseline should contain response metrics");
    assert!(parsed.get("transactions").is_some(), "Baseline should contain transaction metrics");
    assert!(parsed.get("scenarios").is_some(), "Baseline should contain scenario metrics");
    assert!(parsed.get("errors").is_some(), "Baseline should contain error metrics");
    assert!(parsed.get("status_codes").is_some(), "Baseline should contain status code metrics");
...
async fn test_baseline_with_nan_values() {
    ...
    fs::write(temp_path, serde_json::to_string(&baseline_with_nan).unwrap())
...
async fn test_baseline_large_file_handling() {
    ...
    for i in 0..100 { // Reduced from 1000 to keep test reasonable
