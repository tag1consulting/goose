...
pub fn prepare_data_with_baseline_owned<'a>(
    ...
    // Create the Prepare instance directly with the owned baseline
    let prepare = match baseline {
        Some(baseline) => PrepareOwned::new(options, metrics, Some(baseline)),
        None => PrepareOwned::new(options, metrics, None),
    };
    prepare.build()
}

struct PrepareOwned<'m> {
    options: ReportOptions,
    metrics: &'m GooseMetrics,
    baseline: Option<ReportData<'static>>,
    co_data: bool,
