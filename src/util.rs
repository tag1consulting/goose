...
///
/// Used to parse both `--increase-rate` and `--decrease-rate` values.
///
/// # Example
/// ```rust
/// use goose::util;
...
/// assert_eq!(util::parse_rate(Some("1".to_string())), 1.0);
///
/// // Leading decimal returns a proper float.
/// assert_eq!(util::parse_rate(Some(".1".to_string())), 0.1);
///
/// // Valid float string returns a proper float.
/// assert_eq!(util::parse_rate(Some("1.1".to_string())), 1.1);
///
/// // Invalid number with too many decimals returns the defaut of 1.0.
/// assert_eq!(util::parse_rate(Some("1.1.1".to_string())), 1.0);
///
/// // No number returns the defaut of 1.0.
/// assert_eq!(util::parse_rate(None), 1.0);
/// ```
pub fn parse_rate(rate: Option<String>) -> f32 {
...
mod tests {
    ...
    fn test_parse_rate() {
        ...
        assert!((parse_rate(Some("1".to_string())) - 1.0).abs() < f32::EPSILON);
        assert!((parse_rate(Some("1.0".to_string())) - 1.0).abs() < f32::EPSILON);
        assert!((parse_rate(Some(".5".to_string())) - 0.5).abs() < f32::EPSILON);
        assert!((parse_rate(Some("0.5".to_string())) - 0.5).abs() < f32::EPSILON);
        assert!((parse_rate(Some(".12345".to_string())) - 0.12345).abs() < f32::EPSILON);
        assert!((parse_rate(Some("12.345".to_string())) - 12.345).abs() < f32::EPSILON);
        // Defaults to 1.0.
        assert!((parse_rate(None) - 1.0).abs() < f32::EPSILON);
        // Also on invalid input, defaults to 1.0.
        assert!((parse_rate(Some("g".to_string())) - 1.0).abs() < f32::EPSILON);
        assert!((parse_rate(Some("2.1f".to_string())) - 1.0).abs() < f32::EPSILON);
        assert!((parse_rate(Some("1.1.1".to_string())) - 1.0).abs() < f32::EPSILON);
    }
